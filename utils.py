...
def normalize(z,  return_base_values=False, r_base=None):
    """
    Normalize the MRSI data by dividing by the maximum intensity in each frequency channel.
    """
...
def filter_low_power(z, order=6):
    """
    Filter out low power samples from the MRSI data.
    """
...
def plot_freq(z_f):
    """
    Plot the frequency domain representation of the MRSI data.
    """
    T = len(z_f)
...
def save_checkpoint(model, optimizer, config, epoch, checkpoint_dir='checkpoints/training', ex=''):
    """
    Save the checkpoint of the autoencoder model.
    """
...
def load_checkpoint(model, optimizer, checkpoint_path='checkpoints/training', device='cpu'):
    """
    Load the checkpoint of the autoencoder model.
    """
...
def compare_plot(wr_dl, wr_svd, plt_freq_range, org_data=None):
    """
    Plot the comparison of water removal results.
    """
    ppm = point_to_ppm(config_dict['T'])
