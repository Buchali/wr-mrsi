...
#     error = torch.real(original_data[:, meta_range] - wr_data[:, meta_range])
#     mse = torch.sum(error ** 2, dim=-1)  # Compute MSE for each sample (outside water peak region)
#     rmse = torch.sqrt(mse)  # RMSE for each sample

#     # Calculate the magnitude of the original data in the metabolite range
#     original_magnitude = torch.sum(torch.abs(original_data[:, meta_range]) ** 2, dim=-1).sqrt()

#     # Compute relative RMSE (rRMSE) as a ratio
#     rrmse = (rmse / original_magnitude).mean()  # Average rRMSE across all samples
